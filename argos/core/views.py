...
def tabla_estados(request):
    ...
    miembros = list(
        Miembro.objects.filter(activo=True).values(
            "id", "nombre", "grado", "tipo_capita"
        )
    )
    estados = EstadoMensual.objects.filter(anio=year).values(
        "miembro_id", "mes", "estado"
    )
    ...
    for m in miembros:
        row = {
            ...
            "nombre": m["nombre"],
            "grado": m["grado"],
            "tipo_capita": m["tipo_capita"],
        }
        for mes in range(1, 13):
            row[str(mes)] = by_member[m["id"]][mes]
        rows.append(row)
...
def proyeccion_ingresos(request):
    ...
    for m in Miembro.objects.filter(activo=True):
        ...
        if detalle == "por_tipo":
            dt = det_tipo.setdefault(
                m.tipo_capita, {"esperado": Decimal("0"), "proyectado": Decimal("0")}
            )
    ...
    pagado_real = (
        Movimiento.objects.filter(tipo="INGRESO", categoria="Capita").aggregate(total=Sum("monto"))[
            "total"
        ]
        or Decimal("0")
    )
...
def saldo_tesoreria(request):
    ...
    hasta = parse_date(hasta) if hasta else date.today()
...
def resumen_mensual(request):
    ...
    for mes in range(1, 13):
        ...
        res.append(
            {"mes": mes, "ingresos": float(ing), "egresos": float(egr), "neto": float(ing - egr)}
        )
...
def pagar_capitas(request):
    ...
    if fecha_pago_raw:
        ...
        if not fecha_pago:
            return Response({"detail": "fecha_pago inválida. Formato esperado: YYYY-MM-DD."},
    ...
    try:
        ...
        return Response({"detail": "Miembro inexistente."}, status=status.HTTP_404_NOT_FOUND)

    # Si el miembro es EXENTO, no debe pagar
    if str(m.tipo_capita).lower() == "exento":
        return Response({"detail": "El miembro es EXENTO: no corresponde registrar pago de cápitas."},
                        status=status.HTTP_400_BAD_REQUEST)

    # --- Precio vigente por tipo (400 si no hay Suscripción configurada) ---
    precio_por_mes = precio_capita_or_400(m.tipo_capita)

    # --- Validar/armar meses 1..12 (400 si fuera de rango o cantidad <1) ---
    try:
        meses = meses_en_rango_1a12(mes_inicio, cantidad)
    except Exception as e:
        # los helpers ya devuelven ValidationError con detail
        return Response(getattr(e, "detail", {"detail": "Error de validación."}),
                        status=status.HTTP_400_BAD_REQUEST)

    # --- Persistencia atómica ---
    with transaction.atomic():
        # marcar estados como pagada
        for mes in meses:
    ...
    return Response({
        ...
        "meses_pagados": meses,
        "monto": float(precio_por_mes * cantidad),
    })
...
def inicializar_anio(request):
    ...
    try:
        ...
        return Response({"detail": "Parámetro 'anio' inválido."}, status=status.HTTP_400_BAD_REQUEST)
...
def miembros_criticos(request):
    ...
    for m in Miembro.objects.filter(activo=True):
        estados = {
            e["mes"]: e["estado"]
            for e in EstadoMensual.objects.filter(anio=anio, miembro=m).values("mes", "estado")
        }
        if modo == "totales":
